...
@section('content')
    <div class="breadcrumb">
        ...
        <span class="breadcrumb-current">Favorite Saya</span>
    </div>
    ...
    <div class="panel bookmark-list-panel">
        ...
        @forelse($bookmarks as $bookmark)
            ...
            <div class="list-card information-master-card bookmark-master-card">
                @if($type->image_path)
                    <img class="information-master-image" src="{{ asset('storage/' . $type->image_path) }}"
                        alt="Foto {{ $type->name }}">
                @else
                    <div class="information-master-image information-master-placeholder" aria-hidden="true">
                        {{ strtoupper(substr($type->category, 0, 1)) }}</div>
                @endif
                ...
                <div class="information-master-actions">
                    <<<<<<< Updated upstream <a class="btn small information-master-button"
                        href="{{ route('student.applications.create', ['type' => $type->id]) }}">Ajukan</a>
                        <form method="POST" action="{{ route('student.bookmarks.destroy', $type) }}">
                            =======
                            <a class="btn small information-master-button"
                                href="{{ route('student.applications.create', ['type' => $type->uid]) }}">Ajukan</a>
                            <form method="POST" action="{{ route('student.bookmarks.destroy', $type) }}"
                                class="bookmark-ajax-form">
                                >>>>>>> Stashed changes
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bookmark-button active"
        ...
        @empty
            <div class="empty-state bookmark-empty-state" style="border: none; padding: 48px 12px;">
                ...
                <a href="{{ route('student.information') }}" class="btn primary small" style="margin-top: 8px;">Buka
                    Informasi</a>
            </div>
        @endforelse
    </div>
